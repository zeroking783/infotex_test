	https://gitlab.com/zeroking783/infotex_test
# 1. Архив
...
```
## 2. Руководство
Прочитал руководство, в нем рассказыается о разных нюансах сборки SQLite3. Обычно программы компилируют для какого-то тюнинга либо если его не возможности установить (пакета под эту систему нету). Мне тюнить ничего не надо, поэтому используется базовый файл CMakeListst.txt
...
- **CMakeCache.txt** - хранит хэш прошлой сборки, чтобы все собиралось быстрее при незначительных изменениях
- **Makefile** - говорит `make` как нужно компилировать программу
- **CMakeFiles/** - различные настройки и временные файлы для CMake
...
- Dockerfile
		`Dockerfile`
- Readme.txt
		`Readme.txt` (идентичен `README.md`, поэтому лучше сразу открывать его)
